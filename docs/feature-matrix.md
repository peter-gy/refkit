...
| refkit | Python package backed by Rust through PyO3 and maturin. It combines normalized citation rendering with a raw BibTeX editing model. | `Library`, `Entry`, `Style`, `Locale`, `Citation`, `Cite`, `CitationGroup`, `Document`, `RenderedDocument`, `Rendered`, `BibDocument`, `cite`, and `full_bibliography`. | Version 0.0.1. Current API supports Python 3.11 to 3.14, native parsing and rendering, and a 100 percent Python coverage gate. |
