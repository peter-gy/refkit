---
description: Install RefKit PyEmscripten wheels and run the Python API in Pyodide.
---
...
Use the `refkit` Python API in Pyodide. `polars-refkit` supports native CPython on Linux, macOS, and Windows and does not support Pyodide.
...
The xbuild environment pins the runtime application binary interface and build flags. It is a build-environment version rather than the Pyodide product version.
