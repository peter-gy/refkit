...
| Export normalized entries to dictionaries | No public bulk export found. | Partial. Sources are Python objects and mappings. | No CSL normalization. | Yes. `Library.project` returns selected RefKit fields and `polars-refkit.entries` returns the same fields as structs. |
| Export CSL JSON | Native item data uses CSL JSON-like objects. No package-level exporter was found. | Partial. `CiteProcJSON` imports JSON. No exporter found. | No. | No public CSL JSON interchange API. |
