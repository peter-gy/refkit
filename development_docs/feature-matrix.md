...
| --- | --- | --- | --- |
| citeproc-js | JavaScript CSL and CSL-M processor. It renders citation clusters and bibliographies from CSL-like item data supplied by a host application. | `new CSL.Engine(sys, style)`, then `processCitationCluster`, `makeCitationCluster`, `makeBibliography`, `updateItems`, and related state APIs. | Mature processor package. README states the project passes more than 1,300 integration tests. |
...
| refkit | Pure Python package backed by the exact matching `refkit-core` native package. It combines normalized citation rendering with raw BibTeX formatting and editing. | `Library`, `Entry`, `Style`, `Locale`, `Citation`, `Cite`, `CitationGroup`, `Document`, `RenderedDocument`, `Rendered`, `BibDocument`, `TidyOptions`, `TidyResult`, `cite`, `full_bibliography`, `tidy_bibtex`, and `tidy_file`. | Current API supports the Python versions declared in package metadata and rejects a mismatched `refkit-core` version at import time. |
| refkit-core | Rust/PyO3 extension package used by `refkit`. It builds `refkit_core._refkit_core` for CPython and PyEmscripten. | `Library`, `Entry`, `Style`, `Locale`, `Citation`, `Cite`, `CitationGroup`, `Document`, `RenderedDocument`, `Rendered`, `BibDocument`, `TidyOptions`, `TidyResult`, and `build_info`. | Current package builds CPython and PyEmscripten wheels through the release workflow targets. |
| polars-refkit | Polars expression plugin backed by Rust through PyO3, pyo3-polars, and maturin. It parses, formats, inspects, projects, and renders BibTeX rows inside Polars query plans. | `cite`, `cite_each`, `cite_group`, `cite_html`, `full_bibliography_text`, `full_bibliography_html`, `entry_count`, `can_parse`, `has_diagnostics`, `keys`, `entries`, `parse_report`, `diagnostics`, `to_hayagriva_json`, `tidy_bibtex`, `tidy_bibtex_report`, and the `pl.Expr.refkit` namespace. | Current API supports the Python versions declared in package metadata and returns null for row-level parse or formatting failures where the expression has a scalar or list value result. Current package builds CPython and PyEmscripten wheels through the release workflow targets. |

## High-Level Workflows

| Workflow | citeproc-js | citeproc-py | python-bibtexparser | refkit workspace |
| --- | --- | --- | --- | --- |
...
| Formatting control when writing BibTeX | No. | No. | Yes. `BibtexFormat` controls indentation, value alignment, block separator, trailing comma, and failed-block comment text. | Yes. `tidy_bibtex`, `tidy_file`, `BibDocument.tidy`, and `TidyOptions` control spacing, alignment, sorting, wrapping, comments, trailing commas, and value style. |
| LaTeX encode and decode transforms | No. | Partial. The BibTeX adapter parses LaTeX for CSL rendering. | Yes through `LatexEncodingMiddleware` and `LatexDecodingMiddleware`. | Partial. `TidyOptions(escape=...)` controls Unicode-to-LaTeX escaping during BibTeX formatting. |
...
| Month normalization | No. | Partial inside the BibTeX source adapter. | Yes through month middleware. | Partial through normalized parser behavior and `TidyOptions(months=True)` during BibTeX formatting. |
...
| Write `.bib` output | No. | No. | Yes. `write_string` and `write_file`. | Partial. `BibDocument.write` writes field edits while preserving unrelated raw blocks. `tidy_file(..., output=...)` writes formatted BibTeX. Normalized `Library.write` is not exposed. |
