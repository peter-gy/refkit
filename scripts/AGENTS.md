...
`rust_quality_contract.py` checks the shared Rust floor, lint inheritance, native linking features, and dependency-audit configuration across all three workspaces. Its suppression checks cover rustfmt-formatted source, tests, examples, and benches. Keep maintenance-advisory exceptions explicit and duplicate-version exceptions exact, with individual reasons.
