...
1. Prefer in-memory APIs inside a notebook. Use `Library.parse_bibtex`, `Library.parse_yaml`, `BibDocument.parse`, and `tidy_bibtex` when source text is already available.
2. Use `recovery="report"` when the task should retain recoverable entries and inspect `library.diagnostics`. Use strict recovery when malformed input must stop the operation.
...
6. Resolve duplicate raw entries or fields with `get_all` before mutation. Preview `BibDocument.to_bibtex()` before overwriting a file.
7. Inspect `TidyResult.warnings` before accepting canonical formatting, key generation, duplicate detection, or merges.
8. Let RefKit exceptions reach the notebook while diagnosing. Catch a specific error only when the workflow has a concrete recovery path.
