...
| Build Python adapter | `(cd packages/refkit && uv run maturin develop)` | `refkit._native` is installed. |
...
| Python adapter or public API | Rebuild `packages/refkit`, then `make python-lint typecheck test rust` |
