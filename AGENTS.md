...
RefKit exposes bibliography parsing, raw BibTeX editing, Citation Style Language rendering, JavaScript bindings, and Polars expressions through one portable Rust capability core.
...
- `packages/refkit-js` adapts the core to JavaScript through WebAssembly, with shared Node/browser classes and Node filesystem helpers.
...
| `packages/refkit-js` | WebAssembly conversion, JavaScript classes and types, initialization, Node filesystem helpers, and npm artifacts. |
...
- Reuse bibliography inputs and expected semantic results across Python, JavaScript, and Polars when their interfaces expose a capability.
...
| JavaScript bindings | `make js-check` |
...
Pyodide claims require built PyEmscripten wheels and runtime execution through `.github/pyodide`. Native artifacts require path remapping and distribution-contract validation.
...
The `refkit-js` npm package ships JavaScript declarations, ES modules, and the WebAssembly module built from `packages/refkit-js/rust`. Keep browser imports free of Node built-ins. Node filesystem helpers belong in the `refkit-js/node` entry. Validate installed tarballs, consumer types, and browser execution before publication.

Shared concepts and task guides use native VitePress code groups with `python [Python]` and `ts [TypeScript]` fences. Keep one explanation per operation and place runtime setup in the browser, Polars, or Pyodide guide. Validate paired snippets through `npm --prefix packages/refkit-js run test:docs` after building the JavaScript adapter.
