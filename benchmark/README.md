...
The benchmark runner compares `refkit`, `polars-refkit`, `citeproc-py`, and the `bibtexparser` 1.x API through named citation and BibTeX workflows. Each result row records the operation, package version, workload shape, source format, setup policy, timing, Python runtime, OS, CPU, refkit commit, and native build mode.
...
- `source_format`: `bibtex`, `bibtex_rows`, `raw_bibtex`, `dirty_bibtex`, `csl_json`, `none`, `unsupported`, or `unknown`.
...
| --- | --- | --- |
| `bibtex_parse` | `refkit`, `polars-refkit`, `citeproc-py`, `bibtexparser-1.x` | Parse BibTeX into each package's library model or dataframe expression result. The Polars case includes file read, dataframe creation, and plugin execution in the measured operation. |
...
| `bulk_materialization` | `refkit`, `polars-refkit`, `citeproc-py`, `bibtexparser-1.x` | Materialize parsed entries into key and title rows. |
| `library_keys` | `refkit`, `polars-refkit`, `citeproc-py`, `bibtexparser-1.x` | Enumerate all citation keys after setup. |
| `entry_lookup` | `refkit`, `polars-refkit`, `citeproc-py`, `bibtexparser-1.x` | Look up or batch-project a fixed set of entry titles after setup. The Polars case prepares the full dataframe, then filters the fixed lookup key set inside the measured operation. |
| `field_projection` | `refkit`, `polars-refkit`, `citeproc-py`, `bibtexparser-1.x` | Project common scalar fields from entries after setup. Refkit uses `Library.project`. `polars-refkit` uses `bibtex_to_csl_json` over one-entry BibTeX rows, then the adapter projects the same scalar fields as the other packages. |
...
`citeproc-py` render cases use CSL JSON for loaded-library render setup. Its one-off render cases read BibTeX inside the measured operation. `polars-refkit` inspection cases use `bibtex_rows`, a dataframe-shaped source where each row contains one BibTeX entry. Key cases use `bibtex_keys`, count cases use `bibtex_entry_count`, and multi-field projection cases use `bibtex_to_csl_json` before adapter-side projection. `polars-refkit` does not run raw malformed-document recovery. The `source_format` field records these differences per row.
...
- `bibtex_rows`: clean generated BibTeX split into one entry per row for dataframe expression benchmarks.
