"""Use RefKit expressions from code-mode agents."""
...
_DISTRIBUTION_NAME = "polars-refkit"
_SKILL_NAME = "polars-refkit"

AgentPluginError = _agent_plugins.AgentPluginError


def agent_plugin() -> _agent_plugins.Plugin:
    """Return the Agent Plugin installed with this Polars RefKit version.
...
Installed polars-refkit version: {_version(_DISTRIBUTION_NAME)}.

Import `polars_refkit` to register `pl.Expr.refkit`. Use its expressions to
inspect bibliography columns, render citations, and format BibTeX rows.

    import polars_refkit.agent as agent
