...
def _stub_tidy_option_names() -> list[str]:
    ...
    for node in module.body:
        if isinstance(node, ast.ClassDef) and node.name == "TidyOptions":
            ...
            return [arg.arg for arg in init.args.kwonlyargs]
    raise AssertionError("TidyOptions stub not found")


def test_tidy_option_names_match_native_allowlist_and_stub() -> None:
    assert _stub_tidy_option_names() == refkit_core._tidy_option_names


def test_tidy_options_runtime_signature_lists_public_keywords() -> None:
    signature = inspect.signature(rk.TidyOptions)

    assert list(signature.parameters) == refkit_core._tidy_option_names
...
""",
    ...
    assert "second" not in duplicate.bibtex
